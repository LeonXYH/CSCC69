...
{
	//lock the pid, del pid, relase the pid and then call the original exit group
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	//if the syscall is monitoring some pid, then check if the current pid is monitored
	//if the syscall is monitoring all pid, then the monitored list is a blanklist, check if the current pid in blanklist
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	// check if pid negative
	if(pid < 0) {
		return -EINVAL;
	}
	
	// check the case pid = 0 for last two cmd
	...
	//check permissions for first two cmd
	...
	//check permissions for last two cmd
	if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) {
		if(current_uid() != 0) {
			if(pid == 0 ) {
				return -EPERM;
	...
	if(cmd == REQUEST_SYSCALL_INTERCEPT) {
		...
		//check if the syscall alright be intercepted
		if(table[syscall].intercepted == 1) {
			return -EBUSY;
		}
		
		//lock the table and save the original syscall 
		spin_lock(&pidlist_lock);
		...
		//lock the call table and set it writtable
		spin_lock(&calltable_lock);
		set_addr_rw((unsigned long)sys_call_table);
		
		//intercept the syscall and set the call table back to read only
		...
		//set the syscall in table to intercepted status
		spin_lock(&pidlist_lock);
		...
		//lock the table and delete the monitored pid list
		spin_lock(&pidlist_lock);
		destroy_list(syscall);
		
		//change the monitored status and unlock the table
		table[syscall].monitored = 0;
		spin_unlock(&pidlist_lock);
		
		//lock the call table and make it writable
		spin_lock(&calltable_lock);
		set_addr_rw((unsigned long)sys_call_table);
		
		//restore the original syscall and unlock it
		set_addr_ro((unsigned long)sys_call_table);
		spin_unlock(&calltable_lock);
		
		//set the syscall in table to unintercepted status
		spin_lock(&pidlist_lock);
		table[syscall].intercepted = 0;
		spin_unlock(&pidlist_lock);
	} else if (cmd == REQUEST_START_MONITORING) {
		
		//check if the pid alright be monitored
		if(pid != 0 && check_pid_monitored(syscall, pid) == 1) {
			return -EBUSY;
		}
		
		//if pid = 0, lock the table and set monitored status to 2
		if(pid == 0) {
			...
			//else the case is to monitore valid pid, so check if the monitored list is blanklist
			spin_lock(&pidlist_lock);
			if(table[syscall].monitored ==  2) {
				
				//if it is in blanklist, delete it, if it is not in blanklist, then its alright monitored, return EBUSY
				...
				//if the monitored list is not a blanklist, check if the pid alright in the list
				if(check_pid_monitored(syscall, pid) == 1) {
					spin_unlock(&pidlist_lock);
					return -EBUSY;
				} else {
				...
				//set the monitored list status to 1
		...
		//if pid is 0, clear the the monitored pid list if monitored status is not 0
		//if monitored status is 0 return -EINVAL
		if(pid == 0) {
			...
			//if the monitored status is 2, add the pid to blanklist
			spin_lock(&pidlist_lock);
			if(table[syscall].monitored == 2) {
				if(add_pid_sysc(pid, syscall) != 0) {
					spin_unlock(&pidlist_lock);
				...
				//else, delete the pid form monitored pid list
				if(del_pid_sysc(pid, syscall) != 0) {
					spin_unlock(&pidlist_lock);
					return -EINVAL;
				}
...
static int init_function(void) {
	...
	//lock the call table and change it writtable
	spin_lock(&calltable_lock);
	set_addr_rw((unsigned long) sys_call_table);

	// save the original syscall and change to my custom syscall
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	sys_call_table[MY_CUSTOM_SYSCALL] = & my_syscall;

	// save the original exit group and change to my exit group
	orig_exit_group = sys_call_table[__NR_exit_group];
	sys_call_table[__NR_exit_group] = & my_exit_group;
	
	//set call table back to read only and unlock it
	set_addr_ro((unsigned long) sys_call_table);
	spin_unlock(&calltable_lock);

	//init the variables in table
	spin_lock(&pidlist_lock);
...
{      
	...
	//restore the syscall and exit group
	spin_lock(&calltable_lock);
	set_addr_rw((unsigned long)sys_call_table);
